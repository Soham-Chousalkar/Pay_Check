...
import PartTimeTile from "./tiles/PartTimeTile";
import SubscriptionTile from "./tiles/SubscriptionTile";
...
function PanelWrapper({ panel, onDragStart, onDrag, onDragEnd, useRetroStyleGlobal, onStateChange, onDelete, scale, panelOpacity = 60, isInGroup = false }) {
  ...
  const handleDeleteClick = (e) => {
    ...
    const storedPref = localStorage.getItem('paycheck_suppress_delete_confirm');
  ...
  const confirmDelete = () => {
    if (dontShowAgain) {
      localStorage.setItem('paycheck_suppress_delete_confirm', 'true');
    }
    handlePanelDelete(panel.id);
  ...
  return (
    ...
    >
      <div className="main-panel smooth-transition relative mx-auto w-full h-full">
        ...
        {/* Render appropriate tile based on type */}
        {(!panel.state?.type || panel.state.type === 'PART_TIME') && (
          <PartTimeTile
            panelId={panel.id}
