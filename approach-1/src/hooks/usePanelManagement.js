...
export function usePanelManagement(panels, setPanels, addToHistory = null) {
   ...
   */
  const handleAddPanel = useCallback((panelId, side, neighborId, type = 'PART_TIME') => {
    ...
    setPanels((prev) => {
      ...
      const newPanel = { 
        ...
        x, 
        y, 
        title: type === 'PART_TIME' ? 'PayTracker' : (type === 'SUBSCRIPTION' ? 'Subscription' : 'Student Loan'),
        state: { type } // Initialize with type
      };
